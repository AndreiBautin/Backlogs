...
describe('sortItems', () => {
  it('sorts by recently-added, newest first', () => {
    ...
    expect(sortItems([older, newer], 'recently-added').map((i) => i.title)).toEqual([
      'New',
      'Old',
    ])
  })
  ...
  it('sorts alphabetically, case-insensitively', () => {
    ...
    expect(
      sortItems([banana, Apple, cherry], 'alphabetical').map((i) => i.title),
  ...
  it('sorts by recently-completed, newest first, with never-completed items last', () => {
    ...
    const sorted = sortItems(
      [finishedEarlier, neverFinished, finishedLater],
      'recently-completed',
    )
  ...
  it('sorts by recently-updated, newest first', () => {
    const stale = buildItem({ title: 'Stale', lastUpdated: '2026-01-01T00:00:00.000Z' })
    const fresh = buildItem({ title: 'Fresh', lastUpdated: '2026-02-01T00:00:00.000Z' })

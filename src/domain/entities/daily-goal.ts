...
function requireDateKey(value: string): string {
  if (!isDateKey(value)) {
    throw new DomainValidationError(`Invalid date key: ${value}`)
  }
  return value
}
...
export function requireDailyGoal(input: DailyGoalInput): DailyGoal {
  ...
  if (unit.length === 0) {
    throw new DomainValidationError('Daily goal unit is required')
  }
  ...
  if (!Number.isInteger(input.amount) || input.amount < 1) {
    throw new DomainValidationError(
      'Daily goal amount must be a whole number of 1 or more',
    )
  }
...
export function formatDailyGoal(goal: DailyGoal): string {
  const plural =
    goal.amount === 1 || goal.unit.endsWith('s') ? goal.unit : `${goal.unit}s`
  return `${goal.amount.toString()} ${plural}/day`
}

export function getProgressOn(
...
export function applyProgressDelta(
  ...
  if (!Number.isInteger(delta)) {
    throw new DomainValidationError('Progress delta must be a whole number')
  }

  const nextAmount = Math.max(0, getProgressOn(entries, dateKey) + delta)
  const others = entries.filter((entry) => entry.date !== dateKey)
  const next =
    nextAmount === 0 ? others : [...others, { date: dateKey, amount: nextAmount }]

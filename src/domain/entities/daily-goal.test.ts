...
describe('requireDailyGoal', () => {
  ...
  it('rejects an empty unit', () => {
    expect(() => requireDailyGoal({ amount: 1, unit: '   ' })).toThrow(
      DomainValidationError,
    )
  })

  it('rejects an amount below one', () => {
    expect(() => requireDailyGoal({ amount: 0, unit: 'chapter' })).toThrow(
      DomainValidationError,
    )
  })

  it('rejects a fractional amount', () => {
    expect(() => requireDailyGoal({ amount: 1.5, unit: 'chapter' })).toThrow(
      DomainValidationError,
    )
  })
})
...
describe('applyProgressDelta', () => {
  it('adds an entry for a day that has none', () => {
    expect(applyProgressDelta([], '2026-08-19', 1)).toEqual([
  ...
  it('rejects a fractional delta', () => {
    expect(() => applyProgressDelta([], '2026-08-19', 0.5)).toThrow(DomainValidationError)
  })

  it('rejects a malformed date key', () => {
    expect(() => applyProgressDelta([], '19/08/2026', 1)).toThrow(DomainValidationError)
  })
})
